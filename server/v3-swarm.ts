...
export async function v3TiGuyChat(
  ...
  try {
    ...
    const interaction = await TiGuyUnified.getInstance().prepareInteraction(userMessage, userId);
    ...
    if (imageData) {
      ...
      const model = getGeminiModel('gemini-1.5-flash', interaction.staticInstructions);
      if (!model) return "Oups, mon oeil de verre est pété. J'peux pas voir ça! 👁️";
      ...
      const aiResponse = result.response.text() || "J'ai vu de quoi, mais j'sais pas trop comment l'dire! 🦫";

      // Memory Store: Vision Path
      if (userId) {
        TiGuyUnified.getInstance().storeMemory(userId, userMessage, aiResponse).catch(e => console.error("Memory store error:", e));
      }
    ...
    const messages: Array<{ role: "system" | "user" | "assistant", content: string }> = [
      { role: "system", content: `${interaction.staticInstructions}\n\n${interaction.dynamicInstructions}` }
    ];
    ...
    const aiResponse = completion.choices[0]?.message?.content || "Ouin, j'ai eu un bug là. Réessaie!";
