 ...
 */
export async function scoutVideo(filePath: string, postId: string): Promise<VideoScoutResult | null> {
  ...
  try {
    ...
    console.log(`✅ [Scouter] Vidéo ${postId} analysée par Ti-Guy. Buzz: ${scoutData.viralScore}%, Sécurité: ${scoutData.safetyApproved ? 'OK' : 'FAIL'}`);
    return scoutData;

  } catch (error: any) {
    console.error(`❌ [Scouter] Échec de l'analyse pour le post ${postId}:`, error.message);
    return null;
  }
}
