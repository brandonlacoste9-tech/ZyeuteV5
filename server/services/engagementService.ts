 ...
 */
export async function postTiGuyFirstComment(postId: string, videoInfo: { summary: string; tags: string[] }): Promise<void> {
  const model = getGeminiModel("gemini-1.5-flash");
  if (!model) return;

  try {
    // 1. Get Ti-Guy user ID
    const results = await db.select().from(users).where(eq(users.username, 'TiGuyScouter')).limit(1);
    const tiGuy = results[0];
    
    if (!tiGuy) {
        console.warn("[Engagement] User @TiGuyScouter not found. Comment skipped.");
        return;
    }
    ...
    console.log(`💬 [Engagement] Ti-Guy a commenté sur le post ${postId}: "${commentText}"`);

  } catch (error: any) {
    console.error(`❌ [Engagement] Erreur lors du commentaire de Ti-Guy:`, error.message);
  }
}
