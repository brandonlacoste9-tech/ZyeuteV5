...
function buildEmailHtml(content: string, emailType: EmailType): string {
  ...
  const ctaMap: Record<EmailType, { text: string; url: string }> = {
    ...
    onboarding_day1: { text: 'Essayer Ti-Guy Studio →', url: `${appUrl}/ai-studio` },
    onboarding_day3: { text: 'Découvrir du contenu →', url: `${appUrl}/explore` },
    onboarding_day7: { text: 'Voir les plans Premium →', url: `${appUrl}/premium` },
    weekly_digest: { text: 'Voir ton profil →', url: `${appUrl}/profile` },
    upgrade_prompt: { text: 'Passer à Creator Pro →', url: `${appUrl}/premium?plan=silver` },
