...
export async function registerRoutes(
  ...
  // Setup Replit Auth (OIDC with Passport)
  await setupAuth(app);
  ...
  // Get current user (Replit Auth - uses OIDC session)
  ...
  app.get("/api/feed", requireAuth, async (req, res) => {
    try {
      ...
      const posts = await storage.getFeedPosts(req.session.userId!, page, limit);
      res.json({ posts });
    } catch (error) {
  ...
  app.get("/api/posts/:id", async (req, res) => {
    try {
      ...
      if (req.session.userId) {
        isFired = await storage.hasUserFiredPost(req.params.id, req.session.userId);
      }
  ...
  app.post("/api/users/:id/follow", requireAuth, async (req, res) => {
    try {
      ...
      const success = await storage.followUser(req.session.userId!, req.params.id);
  ...
  app.delete("/api/users/:id/follow", requireAuth, async (req, res) => {
    try {
      const success = await storage.unfollowUser(req.session.userId!, req.params.id);
  ...
  app.post("/api/stories/:id/view", requireAuth, async (req, res) => {
    try {
      await storage.markStoryViewed(req.params.id, req.session.userId!);
      res.json({ success: true });
    } catch (error) {
