...
export type VideoSource = 
  ...
  | { type: 'partial-chunks'; chunks: { start: number; end: number; data: ArrayBuffer }[]; mimeType?: string; totalSize?: number };
...
const updateNetworkSpeed = (bytes: number, ms: number) => {
    ...
    networkSpeedEstimate = networkSamples.reduce((a, b) => a + b, 0) / networkSamples.length;
};
 ...
 */
export function usePrefetchVideo(url: string, tier: PreloadTier): UsePrefetchVideoResult {
  ...
  useEffect(() => {
    ...
    const fetchNextChunk = async () => {
        ...
        try {
            ...
            const type = response.headers.get('Content-Type')?.split(';')[0] || 'video/mp4';
            const totalSizeStr = response.headers.get('Content-Range')?.split('/')[1];
            const totalSize = totalSizeStr ? parseInt(totalSizeStr) : undefined;

            videoCache.addChunk(url, buffer, startByte, startByte + buffer.byteLength - 1, type, totalSize);
