...
export async function getFeedPosts(page: number = 0, limit: number = 20): Promise<Post[]> {
  ...
  return (data.posts || [])
    .map(mapBackendPost)
    .filter((post: Post | null): post is Post => post !== null && !!post.id && !!post.media_url);
}
...
=======
  return (data || [])
    .map(mapBackendPost)
    .filter((post: Post | null): post is Post => post !== null && !!post.id && !!post.media_url); // Filter out potential nulls or malformed posts
>>>>>>> d24abee (feat: stabilize video pipeline, integrate Gemini 2.5 Flash-Lite, and upgrade colony intelligence)
}
...
=======
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            // Set processing status for videos
    ...
    // Trigger Video Processing if it's a video
    if (isVideo) {
        const { error: taskError } = await supabase
            ...
            .insert({
                ...
                metadata: { 
                    ...
                    initiator: 'upload_client'
                }
            });
        
        if (taskError) {
            apiLogger.warn("Failed to queue video processing task", taskError);
            // We don't fail the post creation, just warn
        }
    }
...
function mapBackendPost(p: any): Post | null {
    // Return a default safe object or throw/return null if critical data is missing
    // For list mapping, we should ideally filter these out upstream or handle them
    if (!p || !p.id) {
        return null; // Let the caller filter this out
    }
