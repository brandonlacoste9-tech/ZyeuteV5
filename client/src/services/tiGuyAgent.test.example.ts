...
export function validateResponse(response: TiGuyResponse | null): boolean {
  ...
  const checks = [
    { name: 'caption', valid: typeof response.caption === 'string' && response.caption.length > 0 },
    { name: 'emojis', valid: Array.isArray(response.emojis) && response.emojis.length >= 3 && response.emojis.length <= 5 },
    { name: 'tags', valid: Array.isArray(response.tags) && response.tags.length >= 1 && response.tags.length <= 3 },
    { name: 'flagged', valid: typeof response.flagged === 'boolean' },
    { name: 'reply', valid: typeof response.reply === 'string' && response.reply.length > 0 },
  ];
