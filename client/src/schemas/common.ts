...
export const GiftTypeEnum = z.enum(['comete', 'feuille_erable', 'fleur_de_lys', 'feu', 'coeur_or']);
...
const VideoPost = PostBase.extend({
    ...
    processing_status: z.enum(['ready', 'pending', 'processing', 'completed', 'failed']).optional(),
    visual_filter: z.string().optional(),
});

// Discriminated Union
const BasePostSchema = z.discriminatedUnion('type', [
    PhotoPost, 
    VideoPost
]);
...
export const PostSchema = z.preprocess((val: any) => {
    ...
    return {
        ...
        comment_count: val.comments_count || val.comment_count || val.commentCount || 0,
        commentCount: val.comments_count || val.comment_count || val.commentCount || 0,
