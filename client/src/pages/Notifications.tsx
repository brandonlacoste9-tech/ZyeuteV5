...
import { IoFlame, IoChatbubble, IoPersonAdd, IoGift, IoAt, IoEye } from 'react-icons/io5';
...
export const Notifications: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'fire': return <IoFlame className="text-orange-500" />;
      case 'comment': return <IoChatbubble className="text-blue-500" />;
      case 'follow': return <IoPersonAdd className="text-gold-500" />;
      case 'gift': return <IoGift className="text-purple-500" />;
      case 'mention': return <IoAt className="text-green-500" />;
      case 'story_view': return <IoEye className="text-zinc-400" />;
      default: return null;
    }
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-gold-500 font-bold tracking-widest animate-pulse">CHARGEMENT...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in-95 duration-700">
            ...
            <p className="text-leather-400 max-w-xs mx-auto text-sm">
              Restez à l&apos;affût! On vous alertera dès qu&apos;il y a du nouveau.
            </p>
          </div>
        ) : (
            ...
            {notifications.map((notification, index) => (
              <Link
                ...
                className={`block leather-card p-4 rounded-2xl border border-leather-700 hover:border-gold-500 transition-all group relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 delay-[${index * 50}ms] ${
                  !notification.is_read ? 'bg-gold-500/5 shadow-[0_0_15px_rgba(255,191,0,0.05)]' : ''
                }`}
              >
