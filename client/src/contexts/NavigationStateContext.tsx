...
export const NavigationStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveFeedState = useCallback((key: string, state: Omit<FeedState, 'timestamp'>) => {
  ...
  return (
    <NavigationStateContext.Provider value={{ saveFeedState, getFeedState, clearFeedState }}>
      {children}
