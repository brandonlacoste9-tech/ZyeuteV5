...
import { togglePostFire, addComment, followUser, toggleFollow } from '@/services/api'; // Import your API functions
import { logger } from '@/lib/logger';
import { toast } from '@/components/Toast';

const queueLogger = logger.withContext('NetworkQueue');

export type ActionType = 'FIRE_POST' | 'COMMENT_POST' | 'FOLLOW_USER' | 'UNFOLLOW_USER';
...
export const NetworkQueueProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToQueue = useCallback((type: ActionType, payload: any) => {
      ...
      try {
          switch (action.type) {
              ...
              case 'FOLLOW_USER':
                  ...
                  break;
             case 'UNFOLLOW_USER':
                 await toggleFollow(action.payload.followerId, action.payload.followingId, true);
                  break;
              default:
    ...
    for (const action of queue) {
        ...
        if (!success) {
            if (action.retryCount < MAX_RETRIES) {
                ...
                queueLogger.error(`Action ${action.id} failed max retries, dropping.`);
            }
        }
    }
  ...
  return (
    <NetworkQueueContext.Provider value={{ queue, addToQueue, isOnline, processQueue }}>
      {children}
