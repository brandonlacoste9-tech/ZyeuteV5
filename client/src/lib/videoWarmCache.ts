...
class VideoWarmCache {
   ...
   * Add a chunk
   */
  addChunk(url: string, data: ArrayBuffer, start: number, end: number, mimeType?: string, totalSize?: number) {
    ...
    if (currentMemory + data.byteLength > MAX_VIDEO_MEMORY) {
        cacheLogger.warn(`Memory cap reached for ${url.slice(0, 20)}. Evicting chunks...`);
  ...
  private compactChunks(url: string) {
    ...
    for (let i = 1; i < entry.chunks.length; i++) {
        ...
        if (next.start <= current.end + 1) {
            ...
            const newData = new Uint8Array(current.data.byteLength + next.data.byteLength);
  ...
  setFullBlob(url: string, blob: Blob) {
    ...
    cacheLogger.debug(`Promoted to full blob: ${url.slice(0, 30)}... | Size: ${(blob.size / 1024 / 1024).toFixed(2)}MB`);
