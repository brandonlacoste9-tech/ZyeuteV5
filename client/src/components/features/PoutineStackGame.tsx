...
const PoutineStackGame: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            setCurrentX(prev => {
                let next = prev + direction * speed.current;
    ...
    return (
        <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-end overflow-hidden select-none touch-none" onClick={dropLayer}>
            ...
            <div className="absolute top-12 flex flex-col items-center gap-1 z-20 pointer-events-none">
                <span className="text-xs font-mono text-purple-400 tracking-[0.3em] uppercase">Current Stack</span>
            ...
            <div className="relative w-full max-w-md h-[70vh] mb-20 border-x border-slate-900">
                ...
                {/* Stack View */}
                <div className="absolute bottom-0 w-full flex flex-col-reverse transition-transform duration-500" style={{ transform: `translateY(${Math.max(0, (layers.length - 10) * 20)}px)` }}>
                    {layers.map((layer, i) => (
                        <motion.div
                            key={i}
                    ...
                    {!gameOver && (
                        <div
                            ...
                            style={{
                                ...
                                left: `${currentX}%`,
                                backgroundColor: COLORS[LAYER_TYPES[score % LAYER_TYPES.length]],
                                position: 'absolute',
                                top: -20,
                ...
                {gameOver && (
                    ...
                    >
                        ...
                        >
                            ...
                            <h2 className="text-3xl font-black text-white italic uppercase mb-2">Game Over!</h2>
                            <p className="text-slate-400 font-mono text-sm uppercase tracking-widest mb-8">Base Breached</p>

                            <div className="grid grid-cols-2 gap-4 mb-8">
                                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                                    <span className="block text-[10px] text-slate-500 uppercase tracking-tighter">Stack Height</span>
                                    <span className="text-2xl font-bold text-white">{score}</span>
                                </div>
                                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                                    <span className="block text-[10px] text-slate-500 uppercase tracking-tighter">Status</span>
                                    <span className="text-xs font-bold text-purple-400 uppercase">{submitting ? 'Syncing...' : 'Saved'}</span>
                                </div>
                            </div>
