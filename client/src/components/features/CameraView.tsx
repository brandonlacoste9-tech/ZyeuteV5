...
import { IoClose, IoCameraReverseOutline, IoFlashOutline, IoFlashOffOutline } from 'react-icons/io5';
...
export const CameraView: React.FC<CameraViewProps> = ({ onCapture, onClose, mode = 'both' }) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  const mediaRecorderRef = useRef<MediaRecorder | null>(null);
  const [stream, setStream] = useState<MediaStream | null>(null);
  const [isRecording, setIsRecording] = useState(false);
  const [activeMode, setActiveMode] = useState<'photo' | 'video'>(mode === 'video' ? 'video' : 'photo');
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-10 bg-gradient-to-b from-black/60 to-transparent">
        ...
        <div className="flex gap-4">
          ...
          >
            {flash ? <IoFlashOutline size={24} className="text-gold-400" /> : <IoFlashOffOutline size={24} />}
          </button>
          <button 
        ...
        <div className="max-w-md mx-auto flex flex-col items-center gap-8">
          ...
          <div className="relative flex items-center justify-center">
            {activeMode === 'photo' ? (
              ...
              >
                <div className={`transition-all duration-300 ${isRecording ? 'w-8 h-8 bg-red-500 rounded-sm' : 'w-16 h-16 bg-red-600 rounded-full'} shadow-[0_0_20px_rgba(239,68,68,0.3)]`} />
              </button>
            )}
          ...
          <p className="text-zinc-500 text-xs">
            {activeMode === 'video' ? 'Appuie pour filmer' : 'Appuie pour prendre une photo'}
          </p>
        </div>
      </div>
