...
const StreamingDebugOverlay: React.FC<StreamingDebugOverlayProps> = ({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setChunksCount(entry?.type === 'partial' ? entry.chunks.length : (entry?.type === 'full' ? 1 : 0));
    }, 500);
    ...
    <div className="absolute top-4 left-4 z-50 pointer-events-none flex flex-col gap-2 font-mono text-[10px] sm:text-xs">
      ...
      <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-lg p-2 text-white shadow-2xl flex flex-col gap-1 min-w-[180px]">
        <div className="flex justify-between items-center border-b border-white/10 pb-1 mb-1">
          <span className="text-gold-400 font-bold uppercase tracking-wider">Engine v3.1</span>
          <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${tier >= 2 ? 'bg-gold-500 text-black' : 'bg-zinc-700 text-zinc-300'}`}>
            {mode}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span>{activeRequests} / {concurrency}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span>{chunksCount} (Merged: {currentStats.compactionCount})</span>
        </div>

        <div className="flex justify-between">
          <span className="text-zinc-400">Pipeline:</span>
          <span className={isFallback ? 'text-red-400' : (isMse ? 'text-green-400' : 'text-zinc-300')}>
      ...
      <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-lg p-2 text-white shadow-2xl flex flex-col gap-1">
        <div className="flex justify-between items-center border-b border-white/10 pb-1 mb-1">
          <span className="text-blue-400 font-bold uppercase tracking-wider">Metrics</span>
        </div>
        ...
        <div className="flex justify-between gap-4">
          ...
          <span className={stalls > 0 ? 'text-amber-400' : 'text-green-400'}>{stalls} ({stallDuration}ms)</span>
        </div>
      ...
      <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-lg p-2 text-white shadow-2xl overflow-hidden w-[180px]">
        <div className="text-[8px] text-zinc-500 mb-1 uppercase">Buffer Map</div>
        ...
        <div className="flex justify-between mt-1 text-[8px] text-zinc-500 uppercase">
          <span>0B</span>
          <span>{totalSize ? `${(totalSize / 1024 / 1024).toFixed(1)}MB` : '???'}</span>
        </div>
      </div>
    </div>
  );
