...
- The `DATABASE_URL` uses Supabase's connection pooler with **port 5432** (session mode). The `.env.example` shows port 6543 (transaction mode) but the working connection uses port 5432. The backend's `pg` pool forces SSL with `ssl: { rejectUnauthorized: false }`.
