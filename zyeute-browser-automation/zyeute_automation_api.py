...
class TrendRequest(BaseModel):
    ...
    region: str = Field(
        default="all", description="Quebec region: montreal, quebec-city, all"
    )
...
async def discover_trends(request: TrendRequest):
    ...
    try:
        result = await browser_service.discover_quebec_trends(
            platform=request.platform, region=request.region
        )
...
async def analyze_competitor(request: CompetitorRequest):
    ...
    try:
        result = await browser_service.analyze_competitor(
            url=request.url, metrics=request.metrics
        )
        if not result.get("success"):
            raise HTTPException(
                status_code=500,
...
async def create_trend_job(request: JobRequest, background_tasks: BackgroundTasks):
    ...
    return {
        "job_id": job_id,
        "status": "pending",
        "message": f"Trend discovery job started for {request.platform}",
    }
