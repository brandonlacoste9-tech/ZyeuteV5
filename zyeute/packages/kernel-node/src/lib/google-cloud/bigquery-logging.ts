...
export class WaxLedger {
  ...
  constructor(
    projectId: string,
    datasetId: string = "colony_os",
    tableId: string = "infantry_logs",
  ) {
   ...
   */
  async log(entry: InfantryLog): Promise<void> {
    try {
      const row = {
      ...
      console.log(
        `📊 [WAX LEDGER] Logged: ${entry.unit} - ${entry.message.substring(0, 50)}...`,
      );
    } catch (error: any) {
   ...
   */
  async query(whereClause: string, limit: number = 100): Promise<any[]> {
    const query = `
      SELECT *
   ...
   * Get mission summary
   */
  async getMissionSummary(missionId: string): Promise<any> {
    const query = `
      SELECT 
