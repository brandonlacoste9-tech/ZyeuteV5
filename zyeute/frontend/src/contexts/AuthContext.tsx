...
import { getSession, onAuthStateChange, signOut as legacySignOut } from "@/lib/legacySupabase";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        // TODO: Replace with authClient.getSession() once Clerk is integrated
        const initialSession = await getSession();

        trackPerformance("Auth getSession", sessionStart);
    ...
    // TODO: Replace with authClient.onAuthStateChange() once Clerk is integrated
    const {
      data: { subscription },
    } = onAuthStateChange(
  ...
  const logout = async () => {
    setIsLoading(true);
    try {
      // TODO: Replace with authClient.signOut() once Clerk is integrated
      await legacySignOut();
      setIsGuest(false);
      setIsAdmin(false);
      setUser(null);
