...
import { AIDebugBadge } from "../debug/AIDebugBadge";
...
export const TiGuy: React.FC = () => {
  ...
  const handleSendMessage = async (text?: string) => {
    ...
    try {
      ...
      // Store metadata for debug badge
      if (data.metadata) {
        setLastAIMetadata(data.metadata);
      }
            ...
            {messages.map((message) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  {message.sender === "tiguy" && message.id === messages[messages.length - 1]?.id && (
                    <AIDebugBadge metadata={lastAIMetadata} className="self-start" />
                  )}
                <div
                  className={cn(
