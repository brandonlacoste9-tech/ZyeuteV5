...
export const PoutineStackGame: React.FC<{ mockMode?: boolean }> = ({
  mockMode = false,
}) => {
  ...
  useEffect(() => {
    const checkRestrictions = async () => {
      if (mockMode) return; // Skip in playground
  ...
  const submitScore = async () => {
    ...
    if (mockMode) {
      ...
      alert(`Score simulated: ${score}`);
      setIsSubmitting(false);
      navigate("/playground"); // Redirect to playground in mock mode
      return;
    }
    ...
    >
      ...
      <div className="flex-1 relative bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-zinc-900 to-black">
          ...
          <AnimatePresence>
            {pieces.map((p, i) => (
              <motion.div
                ...
                className={`absolute rounded-xl border-2 flex items-center justify-center font-black italic text-xs ${p.type === "fries"
        ...
        {!isGameOver && isPlaying && (
          <div
            className={`absolute top-1/2 -translate-y-1/2 h-10 rounded-xl border-2 transition-colors flex items-center justify-center font-black italic text-xs shadow-2xl ${score % 3 === 0
        ...
        {isGameOver && (
          ...
          >
            ...
            <p className="text-zinc-500 font-bold mb-12 text-center text-xl">
              {t("game.game_over_desc")}{" "}
              {score > 10 ? t("game.good_job") : t("game.bad_job")}
            </p>
        ...
        <div className="absolute inset-0 z-40 bg-black flex flex-col items-center justify-center p-8">
          ...
          <div className="bg-purple-600/20 border border-purple-500/50 px-4 py-1 rounded-full mb-6">
            <span className="text-purple-400 font-bold text-xs tracking-[0.2em] uppercase">
              {t("game.start_edition")}
            </span>
          </div>
          <p className="text-zinc-500 font-bold mb-12 text-center max-w-xs whitespace-pre-line">
            {t("game.start_desc")}
          </p>
          <button
            onClick={startNewGame}
