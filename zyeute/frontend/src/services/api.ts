...
// TODO: Replace with authClient.getAccessToken() once Clerk is integrated
import { getSession } from "@/lib/legacySupabase";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500) {
        ...
        return { data: null, error: "Server Unavailable", status: response.status };
      }
      return { data: null, error: (data && data.error) || "Request failed", status: response.status };
    }
...
export async function getDiscoverPosts(options: {
  type: "tags" | "vibe" | "recommended";
  query?: string;
  vibe?: string;
  limit?: number;
  hiveId?: string;
  ...
  try {
    ...
    if (options.vibe) params.set("vibe", options.vibe);
    if (options.limit) params.set("limit", options.limit.toString());
    if (options.hiveId) params.set("hive", options.hiveId);

    const { data, error, status } = await apiCall<{ posts: Post[] }>(
      `/feed/discover?${params.toString()}`,
    );

    if (error || !data) {
      // Throw error for 401 (Unauthorized) so caller can handle fallback
      if (status === 401 || error?.includes("401") || error?.includes("Unauthorized") || error?.includes("Authentication")) {
        const authError = new Error(error || "Unauthorized");
        (authError as any).status = 401;
        throw authError;
      }
      apiLogger.warn("Discovery feed unavailable, returning empty array");
      return [];
    }
    ...
    // Re-throw 401 errors so caller can handle them
    if ((err as any)?.status === 401 || (err as Error)?.message?.includes("401")) {
      throw err;
    }
    apiLogger.error("Error in getDiscoverPosts:", err);
    return [];
  }
}
...
export async function getExplorePosts(
  ...
  try {
    // TODO: Replace direct Supabase query with repository call once Neon is set up
    // For now, route through Express backend API
    const { data, error } = await apiCall<{ posts: Post[] }>(
      `/explore?page=${page}&limit=${limit}${hiveId ? `&hive=${hiveId}` : ""}`,
    );

    if (error || !data) {
      apiLogger.warn("Explore posts unavailable, returning empty array.");
      return [];
    }
...
export async function createPost(postData: {
  ...
  visualFilter?: string;
  isEphemeral?: boolean;
}): Promise<{ post: Post | null; jobId?: string }> {
  const { data, error } = await apiCall<{ post: Post; jobId?: string }>("/posts", {
    method: "POST",
    body: JSON.stringify({
      ...postData,
  ...
  return { post: mapBackendPost(data.post), jobId: data.jobId };
}
...
export async function firePost(
  postId: string,
): Promise<{ added: boolean; fire_count: number } | null> {
  const { data, error } = await apiCall<{ added: boolean; newCount: number }>(
    `/posts/${postId}/fire`,
    { method: "POST" },
  );
  if (error || !data) return null;
  return { added: data.added, fire_count: data.newCount };
}

export async function unfirePost(
  postId: string,
): Promise<{ success: boolean; fire_count: number } | null> {
  const { data, error } = await apiCall<{ success: boolean; fire_count: number }>(
    `/posts/${postId}/fire`,
    { method: "DELETE" },
  );
  if (error || !data) return null;
  return { success: data.success, fire_count: data.fire_count };
}

export async function markNotInterested(postId: string): Promise<boolean> {
