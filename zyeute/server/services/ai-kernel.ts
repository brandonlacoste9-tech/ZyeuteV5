...
let geminiVision: any = null;
let predictiveService: v1.PredictionServiceClient | null = null;
...
const getGemini = (modelName: 'gemini-pro' | 'gemini-pro-vision') => {
  ...
  console.warn("[AI-Kernel] No GEMINI_API_KEY found. Vertex AI execution requires @google-cloud/vertexai package which is missing.");
  return null;
};
...
export class AIKernelService {
   ...
   */
  static async moderateContent(imageUrl: string): Promise<{ safe: boolean; reason?: string }> {
    return safeExec(async () => {
      ...
      const prompt = "Analyze this image for content moderation. Is it safe for a general audience? Check for nudity, violence, or hate symbols. Return JSON: { \"safe\": boolean, \"reason\": string }";
      ...
      console.log("[AI-Kernel] Moderating content (Mock/Stub for stability)...");
   ...
   */
  static async generateCaption(imageUrl: string, context?: string): Promise<string> {
    return safeExec(async () => {
