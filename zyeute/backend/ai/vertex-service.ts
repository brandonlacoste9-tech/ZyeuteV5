...
import { callGeminiModel, selectModelForRequest } from "./vertex-model-service.js";
import { selectModelWithPolicy, type UseCase } from "./model-policies.js";
 ...
 * Now with Circuit Breaker protection for automatic failover
 */
// Singleton Circuit Breaker instance (shared across requests)
...
export async function generateWithTIGuy(
  ...
  userCredits: number = 0, // Optional: for model selection
  useCase: UseCase = "content", // Optional: use case for policy selection
): Promise<ContentGenerationResponse & { metadata?: { intendedModel: string; actualModel: string; circuitBreakerIntervened: boolean; useCase: string; reason: string } }> {
  ...
  try {
    ...
    // Determine use case from mode if not provided
    const finalUseCase: UseCase = useCase === "content" 
      ? (mode === "customer_service" ? "customer_service" : "content")
      : useCase;

    // Select model using policy system
    const modelSelection = selectModelWithPolicy(finalUseCase, userCredits);
    
    // Get circuit breaker state for the intended model
    const breakerState = breaker.getModelState(modelSelection.intendedModel);
    
    // Re-select with circuit breaker state if policy allows
    const finalSelection = breakerState && breakerState.state === "OPEN" && modelSelection.policy.allowCircuitBreaker
      ? selectModelWithPolicy(finalUseCase, userCredits, {
          model: modelSelection.intendedModel,
          state: breakerState.state,
        })
      : modelSelection;
    
    // Call with circuit breaker protection
    const result = await breaker.callModel(finalSelection.actualModel, {
      prompt: fullPrompt,
      systemInstruction: undefined, // Using prompt-based system instruction
    ...
    return {
      ...
      metadata: {
        intendedModel: finalSelection.intendedModel,
        actualModel: result.modelUsed,
        circuitBreakerIntervened: result.circuitBreakerIntervened || finalSelection.circuitBreakerIntervened,
