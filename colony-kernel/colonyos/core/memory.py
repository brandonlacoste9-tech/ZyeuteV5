...
class BaseMemory:
    ...
    def store(
        self, key: str, value: Any, scope: str = "default", ttl: Optional[int] = None
    ) -> None:  # pragma: no cover - interface
        raise NotImplementedError

    def retrieve(
        self, key: str, scope: str = "default"
...
class SQLiteMemory(BaseMemory):
    ...
    def _initialize(self) -> None:
        with sqlite3.connect(self.path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kv_scope_key ON kv_store(scope, key)"
            )
    ...
    def list_keys(self, scope: str = "default") -> List[str]:
        with self._lock, sqlite3.connect(self.path) as conn:
            rows = conn.execute(
                "SELECT key, expires_at FROM kv_store WHERE scope = ?", (scope,)
            ).fetchall()
...
class HybridMemory:
    ...
    def __init__(
        self, relational: BaseMemory, vector: Optional[VectorMemory] = None
    ) -> None:
