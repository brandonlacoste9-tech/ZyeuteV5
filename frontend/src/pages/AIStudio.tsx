...
export const AIStudio: React.FC = () => {
  ...
  const [videoModel, setVideoModel] = React.useState<"kling" | "wan" | "ltx2">(
    "wan",
  );
  ...
  return (
    <div className="relative h-screen bg-black overflow-hidden flex flex-col items-center justify-center p-4">
      <Header title="Zyeuté Studio Pro" showBack className="absolute top-0 left-0 right-0 z-50 bg-transparent border-none text-white" />
      ...
      <div className="w-full max-w-5xl aspect-[16/10] sm:aspect-video rounded-3xl overflow-hidden bg-zinc-900/40 border border-white/5 relative shadow-[0_0_80px_rgba(0,0,0,0.8)] group">
        {!generatedImage && !generatedVideo && !sourceImage && !isGeneratingImage && !isGeneratingVideo ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 text-center p-8 bg-gradient-to-b from-zinc-900/20 to-black">
            ...
            <h2 className="text-3xl font-black text-white tracking-tight uppercase italic underline-offset-8 decoration-gold-500/50">Studio Créatif</h2>
            <p className="text-zinc-500 max-w-sm text-sm font-medium">L'IA au service de l'influence québécoise. <br/>Commence par une idée.</p>
          </div>
        ) : (
            ...
            {(isGeneratingImage || isGeneratingVideo) ? (
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="space-y-1 text-center">
                  <p className="text-gold-400 font-black uppercase tracking-[0.2em] text-xs">Traitement Pro {videoModel.toUpperCase()}</p>
                  <p className="text-zinc-600 text-[10px] animate-pulse">L'algorithme façonne votre réalité...</p>
                </div>
              </div>
            ) : generatedVideo ? (
              <video 
        ...
        {(generatedImage || generatedVideo) && !isGeneratingImage && !isGeneratingVideo && (
          <div className="absolute top-8 right-8 flex flex-col gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-4 group-hover:translate-x-0">
            <button
              onClick={() => handleDownload((generatedVideo || generatedImage)!, generatedVideo ? "video" : "image")}
            ...
            <button
              onClick={() => handleUseForPost((generatedVideo || generatedImage)!)}
      ...
      <div className="fixed bottom-12 left-1/2 -translate-x-1/2 w-full max-w-4xl px-6 z-50">
        <div className="bg-zinc-950/80 backdrop-blur-3xl rounded-[2.5rem] p-2.5 border border-white/10 flex items-center gap-2 shadow-[0_20px_50px_rgba(0,0,0,0.8)] lg:gap-4 ring-1 ring-white/5">
          ...
          <div className="flex items-center pl-2">
            <button 
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-all border-2 relative group/source ${
                sourceImage ? "border-gold-500 ring-2 ring-gold-500/20" : "border-white/10 hover:border-white/30 bg-white/5"
              } overflow-hidden`}
            >
              {sourceImage ? (
                <>
                  <img src={sourceImage} className="w-full h-full object-cover" alt="source" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/source:opacity-100 flex items-center justify-center text-[10px] text-white font-bold">CLEAR</div>
                </>
              ) : (
          ...
          <div className="flex-shrink-0 animate-pulse hidden sm:block">
            <span className="text-xl filter drop-shadow-[0_0_5px_rgba(255,191,0,0.5)]">✨</span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <input
              ...
              placeholder={activeTab === "video" ? "Décrire l'action et l'ambiance..." : "Entrer un prompt créatif..."}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  activeTab === "video" ? handleGenerateVideo() : handleGenerateImage();
                }
              }}
            />
          </div>
            ...
            >
              {isGeneratingImage || isGeneratingVideo ? (
                ...
                <>
                  ...
                  <span className="text-lg">{activeTab === "video" ? "🎬" : "📸"}</span>
                </>
              )}
            </button>
