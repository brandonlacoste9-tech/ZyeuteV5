...
export const SwarmMap: React.FC = () => {
    ...
    const [selectedPoint, setSelectedPoint] = React.useState<SwarmPoint | null>(null);
            ...
            <main className="flex-grow relative flex flex-col p-4">
                ...
                <div className="flex-grow leather-card stitched rounded-3xl relative overflow-hidden bg-black/40 border border-gold-500/20 shadow-[inset_0_0_50px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="absolute inset-0 opacity-10 pointer-events-none"
                        style={{ backgroundImage: 'radial-gradient(circle, #FFBF00 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
                    ...
                    {isLoading ? (
                        ...
                        <div className="absolute inset-x-8 inset-y-12">
                            ...
                            <svg viewBox="0 0 500 600" className="absolute inset-0 w-full h-full opacity-20 text-gold-500 fill-current">
                            ...
                            {/* Data Points */}
                            {points.map((point) => {
                                const pos = projectPoint(point.location.coordinates[0], point.location.coordinates[1]);
                    ...
                    <div className="absolute top-4 left-4 p-3 bg-black/60 rounded-xl border border-gold-500/30 backdrop-blur-md">
                        <h3 className="text-xs font-bold text-gold-500 uppercase tracking-widest mb-1">Status de la Ruche</h3>
                        <div className="flex items-center gap-2">
                            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                            <span className="text-[10px] text-stone-300 font-mono">EN DIRECT - {points.length} ACTIVITÉS</span>
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex gap-4">
                                    <div className="w-20 h-20 rounded-xl overflow-hidden border border-gold-500/30">
                                        <img src={selectedPoint.thumbnail} alt="" className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex-grow">
                                        <div className="flex justify-between items-start">
                                            <h4 className="font-bold text-white line-clamp-1">{selectedPoint.title || "Observation dans la Ruche"}</h4>
                                            <button onClick={() => setSelectedPoint(null)} className="text-stone-500 text-xl">&times;</button>
                                        </div>
                                        <p className="text-xs text-gold-500/80 mb-2">📍 {selectedPoint.city || selectedPoint.region || "Territoire Québec"}</p>
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center gap-1">
                                                <span className="text-sm">🔥</span>
                                                <span className="text-xs font-bold">{selectedPoint.fireCount}</span>
                                            </div>
                                            <button className="text-[10px] px-3 py-1 bg-gold-gradient text-black font-bold rounded-full uppercase">Accéder</button>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                    <div className="p-3 leather-card rounded-xl border border-gold-500/10 flex flex-col items-center">
                        <span className="text-[10px] uppercase text-stone-500 tracking-tighter">Engagement Total</span>
                        <span className="text-xl font-bold text-white tracking-widest">
                            {points.reduce((acc, p) => acc + p.fireCount, 0).toLocaleString()} 🔥
                        </span>
                    </div>
                    <div className="p-3 leather-card rounded-xl border border-gold-500/10 flex flex-col items-center">
                        <span className="text-[10px] uppercase text-stone-500 tracking-tighter">Territoire Actif</span>
                        <span className="text-xl font-bold text-white tracking-widest">QC-514/418</span>
                    </div>
                </div>
            </main>
