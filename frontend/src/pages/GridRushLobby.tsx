...
import { Zap, Coins, ArrowLeft } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { quickMatch, createInvite } from "@/services/gridRushService";
...
export default function GridRushLobby() {
  ...
  const handleQuickMatch = async () => {
    ...
    if (balance < stake) {
      setError("Pas assez de piasses! Achète des cennes dans la boutique.");
      return;
    }
    ...
    <div className="min-h-screen bg-black leather-overlay text-white p-4 pb-24">
      ...
      <main className="max-w-md mx-auto space-y-6">
        <section className="leather-card rounded-2xl p-5 stitched border border-gold-500/20">
          <h2 className="text-sm font-semibold text-leather-300 uppercase tracking-wider mb-4">
            Mise (cennes)
          </h2>
          ...
          <p className="text-xs text-leather-400 mt-3 text-center">
            Gagnant remporte {stake * 2}¢ · Égalité = remboursement
          </p>
        </section>
