...
export const TiGuyMessaging: React.FC<TiGuyMessagingProps> = ({
  ...
  const sendMessage = useCallback(async () => {
    ...
    setInputValue("");
    const userMsg = { id: `u-${Date.now()}`, from: "user" as const, text };
    ...
    try {
      ...
      const { data } = await apiCall<{ response: string }>("/tiguy/chat", {
        ...
        body: JSON.stringify({
          message: text,
          history,
          context: { userId: currentUserId, username },
        }),
      });
      setMessages((prev) => [
        ...prev,
        {
          id: `tg-${Date.now()}`,
          from: "tiguy" as const,
          text: data?.response ?? "Osti, j'ai eu un problème! Réessaye!",
        },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        {
          id: `tg-err-${Date.now()}`,
          from: "tiguy" as const,
          text: "Câlisse, chu tombé! Réessaye mon ami! 🦫",
        },
      ]);
    } finally {
      setSending(false);
    }
  }, [inputValue, sending, messages, currentUserId, username]);
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="w-10 h-10 rounded flex-shrink-0 flex items-center justify-center press-scale stitched-gold gold-edition-halo disabled:opacity-40"
            style={{
