  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "engagement_update" }, (payload: any) => {
        ...
        if (postIdsRef.current.includes(data.postId)) {
          setState((prev) => ({
            ...
            [data.postId]: {
              ...prev[data.postId],
              ...(data.fireCount !== undefined && { fireCount: data.fireCount }),
              ...(data.commentCount !== undefined && { commentCount: data.commentCount }),
              ...(data.shareCount !== undefined && { shareCount: data.shareCount }),
            },
          }));
        }
      })
      ...
      .on(
        ...
        (payload: any) => {
          const postId = payload.new?.publication_id || payload.old?.publication_id;
