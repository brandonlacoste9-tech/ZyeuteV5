...
export interface VideoIssue {
  type: "source_missing" | "source_403" | "source_404" | "thumbnail_missing" | 
        "processing_stuck" | "codec_unsupported" | "duration_zero" | "corrupted";
 ...
 */
export async function diagnoseVideo(postId: string): Promise<VideoHealthReport> {
  ...
  try {
    ...
    const hasSource = video.media_url || video.hls_url || video.enhanced_url || 
                      video.original_url || video.mux_playback_id;
    ...
    if (video.processing_status === 'processing' || video.processing_status === 'pending') {
      const hoursProcessing = (Date.now() - new Date(video.created_at).getTime()) / (1000 * 60 * 60);
...
export async function fixVideo(postId: string): Promise<VideoDoctorFix> {
  ...
  if (!diagnosis.canAutoFix) {
    return {
      success: false,
      action: "none",
      message: `Cannot auto-fix: ${diagnosis.issues.filter(i => !i.autoFixable).map(i => i.message).join(", ")}`
    };
  }
...
async function generateThumbnail(postId: string): Promise<VideoDoctorFix> {
  ...
  if (thumbnail_url) {
    return { success: true, action: "thumbnail", message: "Thumbnail already exists" };
  }
 ...
 */
async function checkSourceUrl(url: string): Promise<{ ok: boolean; status?: number; error?: string }> {
  try {
...
function generateRecommendations(issues: VideoIssue[], video: any): string[] {
  ...
  for (const issue of issues) {
    switch (issue.type) {
      case "source_missing":
        recommendations.push("❌ Video source is missing - may need to be re-uploaded");
        break;
      case "source_403":
        recommendations.push("🔧 Try routing through media proxy");
        break;
      case "source_404":
        recommendations.push("❌ Source file deleted from storage - needs re-upload");
        break;
      case "processing_stuck":
      ...
      case "thumbnail_missing":
        ...
        break;
      case "duration_zero":
        recommendations.push("⚠️ Video may be corrupted - check file integrity");
        break;
    }
  }
 ...
 */
export async function healthCheckAllVideos(limit = 100): Promise<VideoHealthReport[]> {
  ...
  logger.info(`[VideoDoctor] Health check complete: ${healthy} healthy, ${sick} sick, ${critical} critical, ${dead} dead`);
 ...
 */
export async function autoFixVideos(limit = 50): Promise<{ fixed: number; failed: number; reports: VideoDoctorFix[] }> {
  ...
  logger.info(`[VideoDoctor] Auto-fix complete: ${fixed} fixed, ${failed} failed`);
