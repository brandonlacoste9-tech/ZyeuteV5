...
const POLLO_API_KEY = process.env.POLLO_API_KEY || "pollo_DEvgXk5otVtmUtJLP9e666bbMaSTNGyxjeiUK2VuhdQD";
    ...
    async getTrendingVideos(region: string = 'CA', count: number = 10) {
        try {
            ...
            return response.data.item_list.map((item: any) => ({
                id: item.id,
                video_url: item.video?.play_addr?.url_list?.[0] || item.video?.download_addr?.url_list?.[0],
            ...
            console.error("[TikAPI] Error fetching trending:", error?.message || error);
            return [];
        }
    },
...
export const PolloService = {
    ...
    async generateVideo(params: {
        ...
        try {
            console.log(`[Pollo AI] Generating video with prompt: ${params.prompt.substring(0, 50)}...`);
            ...
            console.error("[Pollo AI] Generation request failed:", error.response?.data || error.message);
    ...
    async getTaskStatus(taskId: string) {
        try {
            const response = await axios.get(`${this.baseUrl}/video/task/${taskId}`, {
            ...
            console.error(`[Pollo AI] Failed to check task ${taskId}:`, error.message);
