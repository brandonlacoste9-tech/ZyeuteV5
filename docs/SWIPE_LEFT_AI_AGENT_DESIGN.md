...
- **Day 180 (workspace):**
  - **Zero state:** When the user swipes left, a blank white screen is intimidating. The Canvas should **pull context from the Feed** (the “passive discovery” side). Example: user was on a restaurant review in the feed → Agent Canvas shows a floating chip/card: _« Tu veux que je réserve une table à [Restaurant]? »_ (or similar in Québec French). So the first thing they see is _context-aware_, not empty.
