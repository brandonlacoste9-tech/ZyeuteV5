-- Grid Rush RPC hardening: row locks, search_path, CHECK, EXECUTE grants
-- Created: 2026-06-08

BEGIN;

-- Enforce non-negative balances at the data layer.
UPDATE user_wallets SET token_balance = 0 WHERE token_balance < 0;

ALTER TABLE user_wallets
  DROP CONSTRAINT IF EXISTS user_wallets_token_balance_nonneg;

ALTER TABLE user_wallets
  ADD CONSTRAINT user_wallets_token_balance_nonneg
  CHECK (token_balance >= 0);

-- Replace bot-match RPC with concurrency-safe wallet locking.
-- Callable only via service_role (backend verifies JWT before calling).
...
BEGIN
  ...
  -- Create wallet if missing, then lock the row for this transaction so
  -- concurrent spends for the same user_id serialize cleanly.
...
REVOKE ALL ON FUNCTION public.grid_rush_create_bot_match(UUID, INT) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.grid_rush_create_bot_match(UUID, INT) FROM anon, authenticated;
