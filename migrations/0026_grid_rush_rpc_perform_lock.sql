...
-- Drop both legacy signatures before recreating (INT,UUID) is canonical.
...
BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Debit already applied; refund before surfacing a friendly error.
      UPDATE public.user_wallets
      SET token_balance = token_balance + p_stake,
          updated_at = NOW()
      WHERE user_id = p_user_id;
...
-- One ACTIVE match per player_1 (blocks double-tap / concurrent bot spawns).
