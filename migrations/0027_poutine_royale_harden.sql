-- Poutine Royale hardening: CHECK constraints, leaderboard index, RLS
-- Created: 2026-06-09
-- Run after 0025_poutine_royale.sql (idempotent).
--
-- RLS model: PUBLIC LEADERBOARD (read-only via PostgREST).
--   - anon/authenticated may SELECT tournaments + scores for active/completed
--     tournaments only (today's board + past daily boards).
--   - No INSERT/UPDATE/DELETE for client roles — all writes go through Express
--     backend (service_role / pooler), preventing score spoofing via REST.
...
-- Score submission is server-only via /api/royale/submit (JWT + pooler role).
