...
const WORKER_URL = Deno.env.get("VIDEO_WORKER_URL") || "https://video-worker-production.up.railway.app";
...
serve(async (req) => {
  try {
    ...
    if (payload.type === "INSERT" && payload.table === "objects" && payload.schema === "storage") {
      ...
      if (record.bucket_id === "raw-uploads" && record.name.match(/\.(mp4|mov|avi|mkv)$/i)) {
        ...
        if (!response.ok) {
          console.error(`[Colony OS] Worker trigger failed: ${response.statusText}`);
          return new Response(JSON.stringify({ error: "Worker failed" }), { status: 500 });
        }

        const data = await response.json();
        console.log(`[Colony OS] Worker triggered successfully:`, data);
        
        return new Response(JSON.stringify({ success: true, workerResponse: data }), {
    ...
    return new Response(JSON.stringify({ message: "Ignored (not a raw video upload)" }), {
      headers: { "Content-Type": "application/json" },
      status: 200,
    });
